...
function CartoonBrowser() {
  return (
    <motion.svg viewBox="0 0 200 150" className="w-64 h-auto drop-shadow-2xl" animate={{ y: [-10, 10, -10], rotate: [0, 2, -2, 0] }} transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}>
      {/* Browser Chrome */}
      <rect x="10" y="10" width="180" height="130" rx="12" fill="white" stroke="#1e293b" strokeWidth="4" />
      <path d="M10 35 L190 35" stroke="#1e293b" strokeWidth="4" />
      <circle cx="25" cy="22" r="4" fill="#ef4444" />
      <circle cx="40" cy="22" r="4" fill="#eab308" />
      <circle cx="55" cy="22" r="4" fill="#22c55e" />
      
      {/* Content Blocks */}
      <rect x="25" y="50" width="80" height="15" rx="4" fill="#f1f5f9" />
      <rect x="25" y="75" width="150" height="8" rx="4" fill="#f8fafc" />
      <rect x="25" y="90" width="120" height="8" rx="4" fill="#f8fafc" />
      
      <motion.rect x="120" y="50" width="55" height="15" rx="4" fill="#14b8a6" animate={{ scale: [1, 1.05, 1] }} transition={{ duration: 1.5, repeat: Infinity }} />
      
      {/* Floating Elements popping out */}
      <motion.g animate={{ y: [-5, 5, -5] }} transition={{ duration: 2, repeat: Infinity }}>
        <circle cx="160" cy="110" r="15" fill="#22c55e" stroke="#1e293b" strokeWidth="3" />
...
export default function WebDevelopmentPage() {
  return (
    <main className="bg-white min-h-screen">
      <section className="relative pt-44 pb-28 bg-teal-50 border-b-4 border-teal-200 overflow-hidden">
        <div className="absolute -top-40 -left-40 w-96 h-96 bg-teal-300/20 rounded-full blur-[100px] animate-morph-blob pointer-events-none" />
        
        <div className="container-custom relative z-10 flex flex-col md:flex-row items-center gap-16">
          <div className="flex-1 space-y-8">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-white border-4 border-teal-200 text-teal-600 font-black text-sm uppercase tracking-widest shadow-sm">
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-6xl md:text-8xl font-black text-slate-800 tracking-tighter leading-[0.9]">
              Pixel <span className="text-teal-500 relative inline-block animate-wiggle delay-500">Perfect.</span>
            </motion.h1>
      ...
      <section className="py-16 bg-teal-500 border-b-8 border-teal-600">
        <div className="container-custom">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             {[
               ...
               <motion.div key={i} whileHover={{ y: -5 }} className="bg-white rounded-[2rem] p-8 text-center border-4 border-teal-700 shadow-[8px_8px_0px_0px_#0f766e]">
                 <div className="text-6xl mb-4 animate-float">{s.emoji}</div>
                 <div className="text-5xl font-black text-teal-600 mb-2">{s.val}</div>
        ...
        <div className="container-custom">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             {[
               ...
               <motion.div key={i} whileHover={{ scale: 1.02 }} className="flex items-start gap-6 bg-white p-8 rounded-[2rem] border-4 border-slate-200 hover:border-teal-300 transition-colors">
                 <div className="w-16 h-16 bg-teal-50 rounded-2xl border-4 border-teal-100 flex items-center justify-center text-teal-500 shrink-0 animate-pulse">
