...
export default function TechStackManagerPage() {
  ...
  if (loading) return <div className="p-8 text-[#A0A0B0] font-black uppercase tracking-widest animate-pulse">Syncing Technology Matrix...</div>;
      ...
      <div className="space-y-8 max-w-4xl mx-auto pb-20">
        ...
        <AdminCard title={editingId === "new" ? "Add Technology" : "Edit Technology"}>
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-4 pt-4 border-t border-[#1E1E2E]">
               <button 
    ...
    <div className="space-y-8 max-w-5xl mx-auto pb-20">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#A0A0B0] font-medium mt-1 uppercase text-[10px] tracking-widest">Technology Integration Protocol</p>
        </div>
              ...
              {stack.length === 0 && (
                <AdminTableRow>
                  <AdminTableCell className="text-center py-20" colSpan={5}>
                    <p className="text-[#3F3F46] font-black uppercase text-xs tracking-widest">No technologies registered</p>
                  </AdminTableCell>
                </AdminTableRow>
              )}
            </AdminTable>
    ...
    <AdminTableRow ref={setNodeRef} style={style}>
      ...
      <AdminTableCell>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-[#0A0A0F] border border-[#1E1E2E] rounded flex items-center justify-center p-1">
            {item.logo_url ? (
              <img src={item.logo_url} alt="" className="w-full h-full object-contain" />
            ) : (
              <Cpu size={14} className="text-[#3F3F46]" />
            )}
          </div>
          <p className="font-black text-white uppercase text-[12px] tracking-wider">{item.name}</p>
        </div>
      </AdminTableCell>
      <AdminTableCell>
